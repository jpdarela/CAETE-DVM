...
contains
   ...
   use omp_lib
      ...
      real(r_8),dimension(npls) :: tra    !transpiration (mm/day) or (mm/s)
      real(r_8),dimension(npls) :: cl2    ! carbon pos-allocation
      real(r_8),dimension(npls) :: cf2    ! carbon pos-allocation
      real(r_8),dimension(npls) :: ca2    ! carbon pos-allocation
      real(r_8),dimension(3,npls) :: day_storage ! D0=3 g m-2
      real(r_8),dimension(npls) :: vcmax            ! µmol m-2 s-1
      real(r_8),dimension(npls) :: specific_la      ! m2 g(C)-1
      real(r_8),dimension(2,npls) :: nupt             ! g m-2 (1) from Soluble (2) from organic
      real(r_8),dimension(3,npls) :: pupt             ! g m-2
      real(r_8),dimension(npls) :: litter_l         ! g m-2
      real(r_8),dimension(npls) :: cwd              ! g m-2
      real(r_8),dimension(npls) :: litter_fr        ! g m-2
      real(r_8),dimension(npls) :: npp2pay          ! G M-2
      real(r_8),dimension(6,npls) :: lit_nut_content  ! Lit_nut_content variables         [(lln),(rln),(cwdn),(llp),(rl),(cwdp)]
      real(r_8),dimension(3,npls) :: storage_out_bdgt ! d0 = 3
      real(r_8),dimension(npls) :: ar_fix_hr ! store plss npp (C) exchanged with N fixer µorganisms GOTO HR
      integer(i_2),dimension(3,npls) :: limitation_status ! D0=3
      integer(i_4), dimension(2,npls) :: uptk_strat        ! D0=2
      INTEGER(i_4), dimension(npls) :: lp ! index of living PLSs/living grasses
      real(r_8), dimension(npls) :: rnpp_aux ! NPP (g m-2 day-1) construction tracking for each living PLS
      ...
      real(r_8), dimension(npls) :: idx_grasses, idx_pdia
   ...
   ! endif
      ...
      !$OMP SCHEDULE(AUTO) &
      !$OMP DEFAULT(SHARED) &
      !$OMP PRIVATE(p, carbon_in_storage, testcdef, sr, dt1, mr_sto, growth_stoc, ar_aux, total_c, c_def_amount, cl_def, ca_def, cf_def)
      do p = 1,nlen
         carbon_in_storage = 0.0D0
         testcdef = 0.0D0
         sr = 0.0D0
         ar_aux = 0.0D0
         dt1 = dt(:,lp(p)) ! Pick up the pls functional attributes list

         call prod(dt1, ocp_wood(lp(p)),catm, temp, soil_temp, p0, w, ipar, rh, emax&
               &, cl1_pft(lp(p)), ca1_pft(lp(p)), cf1_pft(lp(p)), soil_sat, construction(lp(p))&
               &, ph(p), ar(p), nppa(p), laia(p), f5(p), vpd(p), rm(p), rg(p), rc2(p)&
               &, wue(p), c_def(p), vcmax(p), specific_la(p), tra(p))

         evap(p) = penman(p0,temp,rh,available_energy(temp),rc2(p)) !Actual evapotranspiration (evap, mm/day)

         carbon_in_storage = sto_budg(1, lp(p))
         ...
         call allocation (dt1,nppa(p),uptk_costs(lp(p)), soil_temp, w, tra(p)&
            &, mineral_n,labile_p, on, sop, op, cl1_pft(lp(p)),ca1_pft(lp(p))&
            &, cf1_pft(lp(p)),storage_out_bdgt(:,p),day_storage(:,p),cl2(p),ca2(p)&
      ...
      do p = 1, nlen
         ...
         storage_out_bdgt_1(:,lp(p)) = storage_out_bdgt(:,p)
         limitation_status_1(:,lp(p)) = limitation_status(:,p)
         uptk_strat_1(:,lp(p)) = uptk_strat(:,p)
         npp2pay_1(lp(p)) = npp2pay(p)
         rnpp_out(lp(p)) = rnpp_aux(p)
      enddo
