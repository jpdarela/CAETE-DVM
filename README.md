...
In windows you will probably need to modify the System encoding to UTF-8. This can be done in the Control Panel -> Region -> Administrative -> Change system locale -> check the box "Beta: Use Unicode UTF-8 for worldwide language support". After that, restart the computer. This is needed to run the model in windows.
...
Check the current system encoding in powershell:
...
You can download the Visual Studio/build [tools Installer from here](https://visualstudio.microsoft.com/downloads/).

#### Fortran

ifx.exe - Intel fortran compiler:

[Download the fortran essentials](https://www.intel.com/content/www/us/en/developer/tools/oneapi/hpc-toolkit-download.html?packages=fortran-essentials&fortran-essentials-os=windows&fortran-essentials-win=offline).
...
In the windows Makefile the ifx.exe compiler is called. The Makefile must be executed in a [shell with the proper environment](https://www.intel.com/content/www/us/en/docs/oneapi/programming-guide/2023-0/oneapi-development-environment-setup.html)
...
Python Dependencies (see the [requirements for python 3.11](./src/requirements_311_windows.txt) and [requirements for python 3.12 and above](./src/requirements_3xx_last.txt). The following python libraries are required to run the model:
...
out: some text printed to the screen
